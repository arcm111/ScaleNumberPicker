...
public class VerticalRightScale extends ScaleDesign {
    public static final String TAG = "VerticalRightScale";
    ...
    public VerticalRightScale(Attributes attrs) {
        super(attrs);
        this.attrs = attrs;
    ...
    private void drawRulerTicks(Canvas canvas, int cx, int cy, int y0, int y1, int centerOffset) {
        ...
        for (int d = cy + centerOffset; d > y0; d -= attrs.divisionWidth) {
            ...
            label = formatDecimal(curVal);
            p1.set(cx + attrs.divisionTextOffset, d + getDivisionTextHeight() / 2);
            if (curVal <= attrs.maxValue) drawDivisionText(canvas, p1, label);
        ...
        for (int d = cy + centerOffset; d < y1; d += attrs.divisionWidth) {
            ...
            label = formatDecimal(curVal);
            p1.set(cx + attrs.divisionTextOffset, d + getDivisionTextHeight() / 2);
            if (curVal >= attrs.minValue) drawDivisionText(canvas, p1, label);
            curVal -= attrs.divisionValue;
        }
    }
    ...
    protected void notifyDimensionsChanged() {
        ...
        this.cx = getMarginLeft() + getValueTextWidth() + attrs.valueTextOffset;
